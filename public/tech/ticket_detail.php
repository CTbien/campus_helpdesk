...
if (!isset($_SESSION['user']) || !in_array($_SESSION['user']['role'], ['TECH', 'ADMIN'])) {
...
// 1. Initial fetch to validate access and existence
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Re-fetch ticket to get updated data if no error occurred
    if (empty($error)) {
        $ticket = $ticketService->getTicketDetails($ticketId, (int)$user['id'], $user['role']);
    }
}
...
<head>
    ...
    <style>
        ...
        .bg-tech { background-color: #17a2b8; color: white; }
        .bg-admin { background-color: #0d6efd; color: white; }
        .navbar-custom { <?= $user['role'] === 'ADMIN' ? 'background-color: #0d6efd;' : 'background-color: #17a2b8;' ?> }
        .card { border: none; border-radius: 10px; }
        .ticket-info { background-color: #f1f8ff; border-left: 4px solid #17a2b8; padding: 15px; border-radius: 4px; }
    </style>
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark navbar-custom shadow-sm">
        <div class="container">
            <a class="navbar-brand" href="dashboard.php">
                ...
                <span class="badge bg-light <?= $user['role'] === 'ADMIN' ? 'text-primary' : 'text-info' ?> ms-2"><?= $user['role'] ?></span>
            </a>
    ...
    <div class="container py-5">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <a href="dashboard.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Retour à la liste</a>
            <div class="btn-group">
                        ...
                        <?php else: ?>
                            <div class="mb-4">
                                <?php foreach ($ticket['messages'] as $msg): ?>
                                    <div class="card mb-3 <?= $msg['auteur_role'] === 'ETUDIANT' ? '' : 'border-info' ?>">
                                        <div class="card-header <?= $msg['auteur_role'] === 'ETUDIANT' ? 'bg-white' : 'bg-info text-white' ?> py-2 d-flex justify-content-between">
                                            <strong><i class="<?= $msg['auteur_role'] === 'ETUDIANT' ? 'bi bi-person-fill' : 'bi bi-headset' ?>"></i> <?= htmlspecialchars($msg['auteur_nom']) ?></strong>
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm mb-4">
                    <div class="card-body ticket-info">
                        ...
                        <div class="mb-3">
                            <span class="text-muted d-block small fw-bold text-uppercase">Technicien Assigné</span>
                            <div class="mt-1">
                                <?php if ($ticket['assigne_nom']): ?>
                                    <i class="bi bi-person text-info"></i> <?= htmlspecialchars($ticket['assigne_nom']) ?>
                                <?php else: ?>
